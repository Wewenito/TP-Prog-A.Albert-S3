...
class Joueur:
    ...
    def ajout_perso(self, perso : object):
        print(f'La liste contient actuellement {len(self.__personnages)} personnages.')
        if len(self.__personnages) + 1 <= self.__maximum:
            self.__personnages.append(perso)
            print(f'Le personnage {perso.get_attr("pseudo")} a été associé au joueur {self.__nom}\n')
        else:
    ...
    def acces_perso_2(self, nom : str):
        for item in self.__personnages:
            if item.get_attr("pseudo") == nom:
    ...
    def del_perso_1(self, num : int):
        try:
            x = self.__personnages[num].pseudo
    ...
    def del_perso_2(self, nom : str):
        for item in self.__personnages:
            if item.get_attr("pseudo") == nom:
                x = item.get_attr("pseudo")
                self.__personnages.remove(item)
                print(f'Le personnage {x} a été supprimé de la liste du joueur')
                return x
            else:
                pass
